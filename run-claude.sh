...
#
# Modified from https://github.com/icanhasjonas/run-claude-docker
...
PASSTHROUGH_ARGS=()
EXTRA_VOLUMES=()
BASE_IMAGE="${RUN_CLAUDE_BASE_IMAGE:-ubuntu:25.04}"
BUILD_RETRY_COUNT="${RUN_CLAUDE_BUILD_RETRIES:-3}"
...
# Generate --build-arg flags for proxy variables (if provided).
generate_proxy_build_args() {
  local build_args=""
  ...
  if [[ -n "$http_proxy_value" ]]; then
    build_args="$build_args --build-arg HTTP_PROXY=$http_proxy_value --build-arg http_proxy=$http_proxy_value"
  fi

  if [[ -n "$https_proxy_value" ]]; then
    build_args="$build_args --build-arg HTTPS_PROXY=$https_proxy_value --build-arg https_proxy=$https_proxy_value"
  fi

  if [[ -n "$all_proxy_value" ]]; then
    build_args="$build_args --build-arg ALL_PROXY=$all_proxy_value --build-arg all_proxy=$all_proxy_value"
  fi

  if [[ -n "$no_proxy_value" ]]; then
    build_args="$build_args --build-arg NO_PROXY=$no_proxy_value --build-arg no_proxy=$no_proxy_value"
  fi
...
build_image() {
  ...
  PROXY_BUILD_ARGS=$(generate_proxy_build_args)

  # Pull base image first (with retries) to reduce transient registry failures.
  local pull_attempt=1
  while (( pull_attempt <= BUILD_RETRY_COUNT )); do
    if docker pull "$BASE_IMAGE"; then
      break
    fi
    if (( pull_attempt == BUILD_RETRY_COUNT )); then
      echo -e "${RED}Failed to pull base image ${BRIGHT_CYAN}$BASE_IMAGE${RED} after ${BUILD_RETRY_COUNT} attempts.${NC}"
      echo -e "${YELLOW}Tip: configure Docker daemon proxy or set RUN_CLAUDE_BASE_IMAGE to a reachable mirror.${NC}"
      exit 1
    fi
    echo -e "${YELLOW}Base image pull failed (attempt ${pull_attempt}/${BUILD_RETRY_COUNT}). Retrying...${NC}"
  ...
  sed -i.bak "s|__RUN_CLAUDE_BASE_IMAGE__|$BASE_IMAGE|g" "$TEMP_DIR/Dockerfile"
  ...
  while (( build_attempt <= BUILD_RETRY_COUNT )); do
    ...
    if (( build_attempt == BUILD_RETRY_COUNT )); then
      echo -e "${RED}Failed to build Docker image after ${BUILD_RETRY_COUNT} attempts${NC}"
      exit 1
    fi
    echo -e "${YELLOW}Docker build failed (attempt ${build_attempt}/${BUILD_RETRY_COUNT}). Retrying...${NC}"
...
# Install Playwright MCP and CCometixLine via npm
RUN eval "$(fnm env)" && npm install -g @playwright/mcp@latest @cometix/ccline \
	&& mkdir -p ~/.claude/ccline \
	&& ln -sf "$(command -v ccline)" ~/.claude/ccline/ccline
...
# Ensure Claude Code statusline is configured for CCometixLine
CLAUDE_SETTINGS_FILE="$HOME/.claude/settings.json"
mkdir -p "$HOME/.claude"
if [ -f "$CLAUDE_SETTINGS_FILE" ]; then
  if jq '.statusLine = {"type":"command","command":"~/.claude/ccline/ccline","padding":0}' "$CLAUDE_SETTINGS_FILE" > "$CLAUDE_SETTINGS_FILE.tmp" 2>/dev/null; then
    mv "$CLAUDE_SETTINGS_FILE.tmp" "$CLAUDE_SETTINGS_FILE"
    if [ "$RUN_CLAUDE_VERBOSE" = "1" ]; then
      echo "Claude settings.json merged with CCometixLine statusLine"
    fi
  else
    if [ "$RUN_CLAUDE_VERBOSE" = "1" ]; then
      echo "Warning: existing settings.json is not valid JSON, writing minimal CCometixLine settings"
    fi
    echo '{"statusLine":{"type":"command","command":"~/.claude/ccline/ccline","padding":0}}' | jq . > "$CLAUDE_SETTINGS_FILE"
  fi
else
  echo '{"statusLine":{"type":"command","command":"~/.claude/ccline/ccline","padding":0}}' | jq . > "$CLAUDE_SETTINGS_FILE"
  if [ "$RUN_CLAUDE_VERBOSE" = "1" ]; then
    echo "Created Claude settings.json with CCometixLine statusLine"
  fi
fi
